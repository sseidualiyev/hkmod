...
class AutoProfileMod(loader.Module):
    ...
    async def autopfpcmd(self, message):
        ...
        try:
            delete_prev = ast.literal_eval(args[1])
        except (ValueError, SyntaxError):
            return await utils.answer(message, self.strings("invalid_delete", message))

        # download original PFP
        with BytesIO() as pfp_buf:
            await self.client.download_profile_photo("me", file=pfp_buf)
            self.raw_pfp = Image.open(pfp_buf)

        self.rotate_degrees = degrees
        self.delete_previous = delete_prev
        self.degree_accum = 0
        self.pfp_enabled = True
        self.original_pfp_id = photos[0].id

        await utils.answer(message, self.strings("enabled_pfp", message))
        await self.allmodules.log("start_autopfp")

        # start rotation loop
        asyncio.create_task(self._autopfp_loop())

    async def _autopfp_loop(self):
        while self.pfp_enabled:
            self.degree_accum = (self.degree_accum + self.rotate_degrees) % 360
            rotated = self.raw_pfp.rotate(self.degree_accum)

            with BytesIO() as buf:
                rotated.save(buf, format="JPEG")
                buf.seek(0)
                input_file = await self.client.upload_file(buf)
                res = await self.client(functions.photos.UploadProfilePhotoRequest(file=input_file))
                new_photo_id = res.photos[0].id

            # delete original PFP after first rotation
            if self.delete_previous and self.original_pfp_id:
                try:
                    await self.client(
                        functions.photos.DeletePhotosRequest([self.original_pfp_id])
                    )
                except Exception:
                    pass
                self.original_pfp_id = None  # don't delete again
    ...
    async def stopautonamecmd(self, message):
        ...
        await self.client(
            functions.account.UpdateProfileRequest(first_name=self.raw_name.format(time=""))
        )
